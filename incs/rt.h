...
typedef struct					s_param
{
	...
	double						f;
	double						eye[VEC_SIZE];
	double						look[VEC_SIZE];
	double						align[VEC_SIZE];
	double						third[VEC_SIZE];
	double						obj_d;
	double						tmp_d;
	double						tmp_d_cut;
	t_path						*path;
...
**------------------------------------sphere------------------------------------
*/
double							distance_calc(t_object *tmp, double *from,
									double *to);
...
double							distance_to_face1(t_object *tmp, double *from,
									double *to, double face);
double							distance_to_face2(t_object *tmp, double *from,
									double *to, double face);
double							distance_to_face3(t_object *tmp, double *from,
									double *to, double face);
void							display_cube(t_object *objs);
double							distance_to_cube(t_object *t, double *from,
									double *to);
...
void							limits_move(double *o, t_object *obj,
									t_param *param);
...
void							update_normal_quadric(t_quadric *tmp,
									t_path *path);
void							update_normal_cube(t_object *tmp,
									t_path *path);
